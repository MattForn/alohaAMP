...
try:
    ...
    model = stable_baselines3.SAC.load("sac_ConvNext_aloha",env=env)
except:
    # initiate a sac network to learn the task StableBaselines3
    model = stable_baselines3.SAC(CustomSACPolicy, env, verbose=1, buffer_size=1024, batch_size=64)
""" 

from torchsummary import summary

# Create an instance of your feature extractor
feature_extractor = ConvNeXtFeatureExtractor(observation_space=None)

# Move the feature extractor to GPU
feature_extractor = feature_extractor.to("cuda")

# Print the summary with the model on GPU
summary(feature_extractor, input_size=(3, 600, 224), device="cuda")
